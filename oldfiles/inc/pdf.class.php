...
// Purpose of file: Classe PDF permattant de cr�er le contenu du
		...
		if($result = $DB->query($query))
			if($DB->numrows($result) > 0)
			{
				...
				if ($contrat->name() != "" && (count($info_compteur) == 2))
				{
					...
					if (!$contrat->isContratIllim())                          //test si contrat est illimit� ou non 
					{
		...
		*/
		...
		if($result = $DB->query($query))
			if($DB->numrows($result) > 0)
			{
				...
				$this->Cell($col2, 5, utf8_decode($LANG['job'][31]), 1, 0, 'C', true);	// dur�e
		...
		/*
			Construit une requ�te � partir de l'identifiant du ticket ($ID_tracking).
			R�cup�re pour tous les suivis:
			date, cat�gorie, auteur et description.
		*/
